...
const firebaseConfig = {
  apiKey: "AIzaSyDcRSpM7FQqES8sUwF4aEmUkmhrlQBDHqA",
  authDomain: "san-andreas-mapa.firebaseapp.com",
  projectId: "san-andreas-mapa",
  storageBucket: "san-andreas-mapa.appspot.com",
  messagingSenderId: "586483290492",
  appId: "1:586483290492:web:1394cfc214c33c334f6bb3",
...
window.onload = () => {
  ...
  // Po načtení počítáme minimální scale podle obrázku
  const imgNaturalWidth = image.naturalWidth;
  const imgNaturalHeight = image.naturalHeight;

  const containerRatioX = mapBounds.width / imgNaturalWidth;
  const containerRatioY = mapBounds.height / imgNaturalHeight;
  minScale = Math.min(containerRatioX, containerRatioY);
...
map.addEventListener("wheel", e => {
  ...
  scale += e.deltaY * -0.001;
  scale = Math.min(Math.max(minScale, scale), 8);
